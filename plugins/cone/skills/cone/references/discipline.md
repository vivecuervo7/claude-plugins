...
Three patterns govern how the agent should use a cone.

## 1. Peek before you expand

The default move for "I need to look at a file outside my cone" is `git show`, not `expand`. Reading from the object store is free; expanding the cone is permanent within the session.
...
- You need to grep across a directory's contents (`git grep` works on materialised paths only)
- The file needs to exist on disk for a tool you're running (rare in this plugin's scope; that's what the primary checkout is for)
