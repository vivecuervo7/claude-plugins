#!/usr/bin/env node
// wrought-registry.mjs — the snippet registry.
//
// Subcommands:
//   list                      List every snippet across tiers (JSON to stdout)
//   show <name>               Print one snippet's metadata + path
//   reindex                   Regenerate ~/.claude/.vive-claude/wrought/INDEX.md
//   invoke <name> [json-args] Run the snippet against the 'wrought' playwright-cli session
...
//
// Invocation shells out to `playwright-cli -s=wrought run-code "..."`. The
...
const ROOT = process.env.WROUGHT_ROOT || join(homedir(), '.claude/.vive-claude/wrought')
const SESSION = process.env.WROUGHT_SESSION || 'wrought'
const TIERS = ['library', 'staged', 'scratch', 'broken']
const STATS_PATH = join(ROOT, 'stats.json')
const INDEX_PATH = join(ROOT, 'INDEX.md')

function die(msg, code = 1) {
  console.error('wrought-registry:', msg)
  process.exit(code)
}
...
function regenerateIndex() {
  ...
  let md = '# Wrought snippet index\n\n'
  md += '_Auto-generated by `wrought-registry.mjs reindex`. Do not edit by hand._\n\n'
...
function ensureSession() {
  ...
  // `playwright-cli list` formats sessions as `- wrought:` so we need a
  ...
  if (!re.test(r.stdout)) {
    die(`playwright-cli session "${SESSION}" not active — run wrought-session.sh first`, 1)
  }
}
...
function recordAuthoring(name, result) {
  ...
  const sessionId = process.env.WROUGHT_SESSION_ID || null
  const stats = readStats()
...
return await __wrRun(page, args);
  ...
  process.stdout.write(JSON.stringify({
    ...
    tier: found.tier,
    useCount: entry.useCount,
    output: (r.stdout || '').trim().slice(-4000) || null,
  }) + '\n')
}
...
async function main() {
  ...
  switch (cmd) {
    ...
    default:
      die('usage: wrought-registry.mjs <list|show|reindex|invoke|record-authoring|delete> [args...]', 2)
  }
}

main().catch(err => {
  console.error('wrought-registry: unexpected error:', err && err.stack || err)
  process.exit(4)
})
